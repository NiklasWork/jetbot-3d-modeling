...
blockers: gated on OD-003 and on plan-robot package 5 — D-006 defers this decision until the vertical slice has run once
---
...
- **Rotation on the spot gives no parallax.** Overlap is plentiful, triangulation impossible. The JetBot only half escapes this — its camera sits ~5–10 cm ahead of the wheel axis, so a spin traces a small circle. Against 2–5 m of room depth that is a baseline-to-depth ratio of 1–3 % where 5–15 % is wanted.
- **Driving straight at what you are photographing gives no parallax either.** The expansion point sits in the image centre, where displacement is zero.

What does work is **lateral motion relative to the surface being captured** — passing a wall rather than approaching it. The 160° fisheye is the asset that makes this practical: it sees sideways. The best indoor capture path is therefore a closed loop parallel to the walls at constant distance, driven twice at different radii, not an exploration.
...
3. **A whole room by manoeuvres.** A real capture drive in which the human only repositions between manoeuvres and closes at least one loop by returning to an earlier standpoint. Through `jetbot-run.sh` into the pipeline.
...
4. **Label wall-following data.** The weakest primitive is the long wall pass, where the open loop drifts and constant distance is hard to hold by keyboard. Label a steering target per frame in the `road_following` manner — one click per image — on frames already captured in packages 2 and 3. No separate collection drive.
...
5. **Train the regressor on the Nano.** ResNet-18 regression head, trained on the robot as `road_following/train_model.ipynb` does. Training on the device is the point: Torch 1.x and CUDA 10.2 are already there, so no model has to travel. The Mac is the fallback only, and it carries a trap — PyTorch 2.x writes a zip format Torch 1.x cannot read, so it would have to save with `_use_new_zipfile_serialization=False` against a matching torchvision definition.
   *Done when:* a model file on the robot predicts a steering value on a held-out frame.
